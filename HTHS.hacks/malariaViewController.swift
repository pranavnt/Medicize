...
class malariaViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func detect(image: CIImage) {
        guard let model = try? VNCoreMLModel(for: malaria_classifier_1().model) else {
        ...
        let request = VNCoreMLRequest(model: model) { (request, error) in
            ...
            if let firstResult = results.first {
                if firstResult.identifier.contains("Parasit"){
